<template>
              ...
              <td class="border px-4 py-2">
                <div class="flex items-center justify-center gap-4">
                    ...
                    <template v-if="canBeProcessed(request)">
                      ...
                      <button @click="approveRequest(request)" :disabled="processingId === request.id" title="Approve"
                              class="text-green-500 hover:text-green-700 transition disabled:text-gray-300">
                          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                              <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                          </svg>
                      </button>
                      
                      <button @click="declineRequest(request)" :disabled="processingId === request.id" title="Decline"
                              class="text-red-500 hover:text-red-700 transition disabled:text-gray-300">
                          <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                  ...
                  <button v-if="canBeCancelled(request)" @click="cancelRequest(request)" :disabled="processingId === request.id" title="Cancel Permohonan"
                          class="text-gray-500 hover:text-gray-700 transition disabled:text-gray-300">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
  ...
  <div v-if="showAssignModal && requestToAssign" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
    <div class="bg-white w-full max-w-lg p-6 rounded-lg shadow-lg">
      <h2 class="text-xl font-bold mb-4">Alokasikan Barang untuk Permohonan</h2>
      
      <form @submit.prevent="confirmAndAssignItem" class="space-y-4">
        <div class="p-4 bg-gray-100 rounded-md border text-sm">
          <p class="mb-1"><strong>Pemohon:</strong> {{ requestToAssign.requester.name }}</p>
          <p><strong>Tipe Barang Diminta:</strong> <span class="font-semibold capitalize">{{ requestToAssign.item_type }}</span></p>
        </div>

        <div>
          <label for="assign-item" class="block font-medium">Pilih Barang yang Tersedia</label>
          <select id="assign-item" v-model="selectedItemId" class="w-full border rounded px-3 py-2 bg-white" required>
              <option :value="null" disabled>
                  {{ availableItems.length > 0 ? 'Pilih satu barang...' : 'Memuat atau tidak ada barang tersedia...' }}
              </option>
              <option v-for="item in availableItems" :key="item.id" :value="item.id">
                  {{ item.brand }} - (Kode: {{ item.code }})
              </option>
          </select>
          <p v-if="!loadingAvailableItems && availableItems.length === 0" class="text-sm text-yellow-700 bg-yellow-100 p-2 rounded-md mt-2">
              Tidak ada barang dengan tipe '{{ requestToAssign.item_type }}' yang berstatus AVAILABLE saat ini.
          </p>
        </div>

        <div class="mt-6 flex justify-end gap-4">
          <button @click="closeAssignModal" type="button" class="px-4 py-2 bg-gray-300 text-gray-700 rounded hover:bg-gray-400">
            Batal
          </button>
          <button type="submit" :disabled="isAssigning || !selectedItemId || availableItems.length === 0" class="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700 disabled:opacity-50">
            {{ isAssigning ? 'Memproses...' : 'Setujui & Alokasikan' }}
          </button>
        </div>
      </form>
    </div>
